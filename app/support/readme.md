# ✅ **`app_base/app/support/README.md`**

```markdown
# PTEKWPDEV — Support Layer

The `app/support` directory contains the shared tooling, templates, and
meta‑operations used by the PTEKWPDEV bootstrap environment. These files are not
part of the runtime application; instead, they provide deterministic,
contributor‑safe scaffolding for generating and validating scripts across the
toolchain.
...
Each template provides a self‑contained block of logic that can be injected into
generated scripts by the `script_*` utilities.

### **1. `template.error.sh`**
Provides the canonical error‑handling block:
...
Provides helper‑function loading:
...
Used when generators include app‑config (`-a`).
...
Used when generators include project‑config (`-p`).

---

## 🛠 Script Generators

The `app/support/bin` directory contains meta‑scripts that consume these
...
2. Inject them into generated scripts inside DO‑NOT‑EDIT blocks
3. Maintain deterministic, contributor‑safe script structure
4. Enforce canonical include ordering and mutation‑aware behavior

---

## 🔒 Mutation‑Aware Blocks

All generated include blocks are wrapped in:

```
# ========================================================================
# DO NOT EDIT THIS BLOCK — generated by <tool>
# To modify includes, re-run the generator with updated flags.
# ========================================================================
```

This ensures contributors never hand‑edit generated logic and can always
